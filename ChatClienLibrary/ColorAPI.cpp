...
// 색상 반전
void ColorAPI::APIInverse(IppRgbImage& img)
{
...
// 히스토그램
void ColorAPI::APIHistogram(IppRgbImage& img, float Rhisto[256], float Ghisto[256], float Bhisto[256])
{
	int size = img.GetSize();
	RGBBYTE* p = img.GetPixels();
...
// 히스토그램 스트레칭
void ColorAPI::APIHistogramStretching(IppRgbImage& img)
{
	int size = img.GetSize();
	RGBBYTE* p = img.GetPixels();
...
// 히스토그램 균등화
void ColorAPI::APIHistogramEqualization(IppRgbImage& img)
{
	int size = img.GetSize();
	RGBBYTE* p = img.GetPixels();
	...
	APIHistogram(img, Rhist, Ghist, Bhist);
...
// 덧셈 연산
bool ColorAPI::APIAdd(IppRgbImage& img1, IppRgbImage& img2, IppRgbImage& img3)
{
	int w = img1.GetWidth();
...
// 뺄셈 연산
bool ColorAPI::APISub(IppRgbImage& img1, IppRgbImage& img2, IppRgbImage& img3)
{
	int w = img1.GetWidth();
	int h = img1.GetHeight();
...
// 평균 연산 수행
bool ColorAPI::APIAve(IppRgbImage& img1, IppRgbImage& img2, IppRgbImage& img3)
{
	int w = img1.GetWidth();
	int h = img1.GetHeight();
...
// 차이 계산
bool ColorAPI::APIDiff(IppRgbImage& img1, IppRgbImage& img2, IppRgbImage& img3)
{
	int w = img1.GetWidth();
	int h = img1.GetHeight();
...
// AND 연산
bool ColorAPI::APIAND(IppRgbImage& img1, IppRgbImage& img2, IppRgbImage& img3)
{
	int w = img1.GetWidth();
	int h = img1.GetHeight();
...
// OR 연산
bool ColorAPI::APIOR(IppRgbImage& img1, IppRgbImage& img2, IppRgbImage& img3)
{
	int w = img1.GetWidth();
	int h = img1.GetHeight();
...
// 평균 값 필터
void ColorAPI::APIFilterMean(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
...
// 가중 평균값 필터링
void ColorAPI::APIFilterWeightedMean(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
// 라플라시안 필터
void ColorAPI::APIFilterLaplacian(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
// 언샤프 마스크 필터
void ColorAPI::APIFilterUnsharpMask(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
// 하이부스트 필터
void ColorAPI::APIFilterHighboost(IppRgbImage& imgSrc, IppRgbImage& imgDst, float alpha)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
// 가우시안 임의 잡음 생성
void ColorAPI::APINoiseGaussian(IppRgbImage& imgSrc, IppRgbImage& imgDst, int amount)
{
...
// 소금& 후추 잡음 생성
void ColorAPI::APINoiseSaltNPepper(IppRgbImage& imgSrc, IppRgbImage& imgDst, int amount)
{
	int size = imgSrc.GetSize();
...
// 미디언 필터
void ColorAPI::APIFilterMedian(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
// 로버츠 엣지 검출
void ColorAPI::APIEdgeRoberts(IppRgbImage& img, IppRgbImage& imgEdge)
{
	int w = img.GetWidth();
	int h = img.GetHeight();
...
// 소벨 엣지 검출
void ColorAPI::APIEdgeSobel(IppRgbImage& img, IppRgbImage& imgEdge)
{
	int w = img.GetWidth();
	int h = img.GetHeight();
...
void ColorAPI::APIResizeNearest(IppRgbImage& imgSrc, IppRgbImage& imgDst, int nw, int nh)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void ColorAPI::APIResizeBilinear(IppRgbImage& imgSrc, IppRgbImage& imgDst, int nw, int nh)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
// 모폴로지 연산
void ColorAPI::APIMorphologyColorErosion(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
...
// 모폴로지 팽창
void ColorAPI::APIMorphologyColorDilation(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
...
// 영상의 열기
void ColorAPI::APIMorphologyColorOpening(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
	IppRgbImage imgTmp;
	APIMorphologyColorErosion(imgSrc, imgTmp);
	APIMorphologyColorDilation(imgTmp, imgDst);
}

// 영상의 닫기
void ColorAPI::APIMorphologyColorClosing(IppRgbImage& imgSrc, IppRgbImage& imgDst)
{
	IppRgbImage imgTmp;
	APIMorphologyColorDilation(imgSrc, imgTmp);
	APIMorphologyColorErosion(imgTmp, imgDst);
}

void ColorAPI::APIResizeCubic(IppRgbImage& imgSrc, IppRgbImage& imgDst, int nw, int nh)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
	...
	for (j = 0; j < nh; j++)
		for (i = 0; i < nw; i++)
		{
			...
			GeometryAPI geo;
			Rv1 = geo.cubic_interpolation(pSrc[y1][x1].r, pSrc[y1][x2].r, pSrc[y1][x3].r, pSrc[y1][x4].r, p);
			Rv2 = geo.cubic_interpolation(pSrc[y2][x1].r, pSrc[y2][x2].r, pSrc[y2][x3].r, pSrc[y2][x4].r, p);
			Rv3 = geo.cubic_interpolation(pSrc[y3][x1].r, pSrc[y3][x2].r, pSrc[y3][x3].r, pSrc[y3][x4].r, p);
			Rv4 = geo.cubic_interpolation(pSrc[y4][x1].r, pSrc[y4][x2].r, pSrc[y4][x3].r, pSrc[y4][x4].r, p);

			Gv1 = geo.cubic_interpolation(pSrc[y1][x1].g, pSrc[y1][x2].g, pSrc[y1][x3].g, pSrc[y1][x4].g, p);
			Gv2 = geo.cubic_interpolation(pSrc[y2][x1].g, pSrc[y2][x2].g, pSrc[y2][x3].g, pSrc[y2][x4].g, p);
			Gv3 = geo.cubic_interpolation(pSrc[y3][x1].g, pSrc[y3][x2].g, pSrc[y3][x3].g, pSrc[y3][x4].g, p);
			Gv4 = geo.cubic_interpolation(pSrc[y4][x1].g, pSrc[y4][x2].g, pSrc[y4][x3].g, pSrc[y4][x4].g, p);

			Bv1 = geo.cubic_interpolation(pSrc[y1][x1].b, pSrc[y1][x2].b, pSrc[y1][x3].b, pSrc[y1][x4].b, p);
			Bv2 = geo.cubic_interpolation(pSrc[y2][x1].b, pSrc[y2][x2].b, pSrc[y2][x3].b, pSrc[y2][x4].b, p);
			Bv3 = geo.cubic_interpolation(pSrc[y3][x1].b, pSrc[y3][x2].b, pSrc[y3][x3].b, pSrc[y3][x4].b, p);
			Bv4 = geo.cubic_interpolation(pSrc[y4][x1].b, pSrc[y4][x2].b, pSrc[y4][x3].b, pSrc[y4][x4].b, p);

			Rv = geo.cubic_interpolation(Rv1, Rv2, Rv3, Rv4, q); // 각 행의 보간된 점들을 다시 보간하여 최종적으로 보간된 값을 구한다.
			Gv = geo.cubic_interpolation(Gv1, Gv2, Gv3, Gv4, q); // 각 행의 보간된 점들을 다시 보간하여 최종적으로 보간된 값을 구한다.
			Bv = geo.cubic_interpolation(Bv1, Bv2, Bv3, Bv4, q); // 각 행의 보간된 점들을 다시 보간하여 최종적으로 보간된 값을 구한다.
...
//-------------------------------------------------------------------------
void ColorAPI::APIRGB_TO_HSI(double R, double G, double B, double& H, double& S, double& I)
{
	I = (R + G + B) / 3;
...
// R, G, B, H, S, I 의 범위는 [0, 1]
//-------------------------------------------------------------------------
void ColorAPI::APIHSI_TO_RGB(double H, double S, double I, double& R, double& G, double& B)
{
	if (I == 0.0) // 검정색
	{
		R = 0.0;
...
//-------------------------------------------------------------------------
void ColorAPI::APIRGB_TO_YUV(BYTE R, BYTE G, BYTE B, BYTE& Y, BYTE& U, BYTE& V)
{
...
// R, G, B, Y, U, V 의 범위는 [0, 255]
//-------------------------------------------------------------------------
void ColorAPI::APIYUV_TO_RGB(BYTE Y, BYTE U, BYTE V, BYTE& R, BYTE& G, BYTE& B)
{
...
// RGB 색상 평면 나누기
void ColorAPI::APIColorSplitRGB(IppRgbImage& imgColor, IppByteImage& imgR, IppByteImage& imgG, IppByteImage& imgB)
{
...
// HSI 색상 평면 나누기
void ColorAPI::APIColorSplitHSI(IppRgbImage& imgColor, IppByteImage& imgH, IppByteImage& imgS, IppByteImage& imgI)
{
	int w = imgColor.GetWidth();
	int h = imgColor.GetHeight();
	int size = imgColor.GetSize();
...
// YUV 색상 평면 나누기
void ColorAPI::APIColorSplitYUV(IppRgbImage& imgColor, IppByteImage& imgY, IppByteImage& imgU, IppByteImage& imgV)
{
	int w = imgColor.GetWidth();
	int h = imgColor.GetHeight();
	int size = imgColor.GetSize();
	...
	for (int i = 0; i < size; i++)
	{
		APIRGB_TO_YUV(pColor[i].r, pColor[i].g, pColor[i].b, pY[i], pU[i], pV[i]);
	}
}

// RGB 색상 평면 합치기
bool ColorAPI::APIColorCombineRGB(IppByteImage& imgR, IppByteImage& imgG, IppByteImage& imgB, IppRgbImage& imgColor)
{
	int w = imgR.GetWidth();
...
// HSI 색상 평면 합치기
bool ColorAPI::APIColorCombineHSI(IppByteImage& imgH, IppByteImage& imgS, IppByteImage& imgI, IppRgbImage& imgColor)
{
	int w = imgH.GetWidth();
	...
	for (int i = 0; i < size; i++)
	{
		...
		APIHSI_TO_RGB(hh, ss, ii, r, g, b); // 트루 컬러 영상으로 출력해야 하기 때문에 변환해준다.
...
// YUV 색상 평면 합치기
bool ColorAPI::APIColorCombineYUV(IppByteImage& imgY, IppByteImage& imgU, IppByteImage& imgV, IppRgbImage& imgColor)
{
	int w = imgY.GetWidth();
	...
	for (int i = 0; i < size; i++)
	{
		APIYUV_TO_RGB(pY[i], pU[i], pV[i], pColor[i].r, pColor[i].g, pColor[i].b);
	}

	return true;
}

void ColorAPI::APIColorEdge(IppRgbImage& imgSrc, IppByteImage& imgEdge)
{
	IppByteImage imgY, imgU, imgV;
	APIColorSplitYUV(imgSrc, imgY, imgU, imgV);

	IppByteImage edgeY, edgeU, edgeV;
	// 프리윗 엣지 검출
	FeatureAPI feat;
	feat.APIEdgePrewitt(imgY, edgeY);
	feat.APIEdgePrewitt(imgU, edgeU);
	feat.APIEdgePrewitt(imgV, edgeV);
