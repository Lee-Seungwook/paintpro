...
BOOL CBrightnessContrastDlg::OnInitDialog()
{
	...
	// 밝기 조절 슬라이더 컨트롤 초기화
	m_sliderBrightness.SetRange(-255, 255); // 슬라이더 컨트롤이 가리킬수 있는 값의 범위 지정
	m_sliderBrightness.SetTicFreq(32); // 슬라이더 컨트롤 눈금 간격
	m_sliderBrightness.SetPageSize(32); // 왼쪽 버튼으로 클릭했을때 움직일 값
...
void CBrightnessContrastDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	// 발기 조절 슬라이드바에서 발생한 WM_HSCROLL 메시지 처리
