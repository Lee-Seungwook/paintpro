...
void CResizeDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_ASPECT_RATIO, m_bAspecRatio);
...
BOOL CResizeDlg::OnInitDialog()
{
	...
	((CSpinButtonCtrl*)GetDlgItem(IDC_SPIN_WIDTH))->SetRange(1, 4096); // 스핀버튼의 범위를 지정
...
void CResizeDlg::OnBnClickedAspectRatio()
{
	...
	if (m_bAspecRatio)
	{
		m_nNewHeight = m_nNewWidth * m_nOldHeight / m_nOldWidth; // 높이의 비율을 적용하여 자동으로 값을 설정
		UpdateData(FALSE);
	}
}
